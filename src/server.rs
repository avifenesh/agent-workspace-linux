...
impl AgentWorkspaceLinux {
    ...
    #[tool(
        ...
        description = "Start a profile-backed isolated workspace, optionally wait for profile setup, optionally kill timed-out setup commands, and launch that profile's startup apps in one operation after setup succeeds.",
        annotations(
            read_only_hint = false,
            destructive_hint = false,
    ...
    #[tool(
        ...
        description = "List apps launched inside an isolated agent workspace, optionally filtering by app id/pid/name, app name substring, command substring, profile id, or running/stopped state.",
        annotations(
            read_only_hint = true,
            destructive_hint = false,
            idempotent_hint = true,
    ...
    #[tool(
        ...
        description = "Read the workspace-local IPC event log. Use since_sequence to poll events after a previously seen sequence number. Sensitive typed text is recorded only as metadata such as character count.",
        annotations(
            read_only_hint = true,
            destructive_hint = false,
            idempotent_hint = true,
