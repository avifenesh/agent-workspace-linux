...
impl AgentWorkspaceLinux {
    pub fn new(permissions: McpPermissionState) -> Self {
        Self { permissions }
    }
}
...
impl AgentWorkspaceLinux {
    ...
    #[tool(
        ...
        description = "Return the spawn-time MCP permission ceiling for this server process. If configured/restricted, clients may narrow network, file mounts, or launchable apps but cannot broaden them through any MCP tool.",
        annotations(
            read_only_hint = true,
            destructive_hint = false,
            idempotent_hint = true,
    ...
    fn profile_put(
        ...
        Json(
            self.permissions
                ...
                .and_then(|_| profile::put_profile(params.profile, params.replace, params.dry_run))
    ...
    fn profile_import(
        ...
        Json(match profile::read_profile_json_file(&params.json_path) {
            Ok(requested_profile) => self
                .permissions
                .validate_profile(&requested_profile)
                .and_then(|_| {
                    profile::put_profile(requested_profile.clone(), params.replace, params.dry_run)
                })
                .unwrap_or_else(|error| {
                    profile::ProfilePutResult::error(
                        requested_profile,
